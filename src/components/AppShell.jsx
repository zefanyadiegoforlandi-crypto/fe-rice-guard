...
const NAV_ITEMS = [
  ...
  { href: '/history', label: 'History', icon: 'fa-clock-rotate-left', requiresAuth: true },
];
...
export default function AppShell({ children }) {
  ...
  // Filter nav items berdasarkan auth status
        ...
        <div className="px-5 pt-5 pb-3">
          <div className="flex items-center gap-3 p-3 rounded-2xl bg-gradient-to-r from-[var(--mint)] to-white">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-[var(--forest)] truncate">{user?.name || 'Pengguna'}</p>
              <p className="text-[11px] text-[var(--muted)]">Petani</p>
            </div>
          </div>
        </div>
